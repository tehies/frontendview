...
const ProductPage = () => {
    ...
    useEffect(() => {
        const fetchSkuAndPrice = async () => {
            try {
                // Fetch SKU details
                const skuResponse = await axios.get(`https://vtex-backend-791k.onrender.com/sku/${skuId}`);
                // const skuResponse = await axios.get(`https://vtex-backend.onrender.com/sku/${skuId}`);
                setSku(skuResponse.data);

                // Fetch Pricing details
                const pricingResponse = await axios.get(`https://vtex-backend-791k.onrender.com/pricing/${skuId}`);
